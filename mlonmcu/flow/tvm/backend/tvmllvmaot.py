...
class TVMLLVMAOTBackend(TVMBackend):
    FEATURES = {
        *TVMBackend.FEATURES,
        "debug_arena",
        "unpacked_api",
    ...
    DEFAULTS = {
        **TVMBackend.DEFAULTS,
        "arena_size": None,  # Detemined automatically
        "debug_arena": False,
        "unpacked_api": False,
        "alignment_bytes": 4,
    }
    ...
    def __init__(self, runtime="crt", fmt="mlf", features=None, config=None):
        super().__init__(target="llvm", executor="aot", runtime=runtime, fmt=fmt, features=features, config=config)
    ...
    def get_tvmc_compile_args(self, out, dump=None):
        return super().get_tvmc_compile_args(out, dump=dump) + get_tvmaot_tvmc_args(
            self.alignment_bytes, self.unpacked_api, target=self.target, runtime=self.runtime
        )
