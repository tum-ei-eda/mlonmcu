 ...
 public:
  ...
  void push_back(const Result& entry) {
    if (size() >= kMaxResults) {
      ...
      printf("Couldn't push_back latest result, too many already!\n");
      return;
    }
    size_ += 1;
    back() = entry;
  ...
  Result pop_front() {
    if (size() <= 0) {
      ...
      printf("Couldn't pop_front result, none present!\n");
      return Result();
    }
    Result result = front();
    front_index_ += 1;
  ...
  Result& from_front(int offset) {
    if ((offset < 0) || (offset >= size_)) {
      ...
      printf("Attempt to read beyond the end of the queue!\n");
      offset = size_ - 1;
    }
...
class RecognizeCommands {
 public:
  ...
  //                                   bool* is_new_command);
  int ProcessLatestResults(const int8_t* latest_results,
                                    const int32_t current_time_ms,
                                    const char** found_command, uint8_t* score,
                                    bool* is_new_command);
