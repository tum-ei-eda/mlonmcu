...
void mlif_run() {
  ...
  while (mlif_request_input(mlif_input_ptr(input_num), mlif_input_size(input_num)) || remaining) {
    if (input_num == mlif_inputs() - 1) {
      mlif_invoke();
      for (size_t i = 0; i < mlif_outputs(); i++) {
        mlif_handle_result(mlif_output_ptr(i), mlif_output_size(i));
      }
      input_num = 0;
