...
class MyPostprocess(SessionPostprocess):
    ...
    def __init__(self, features=None, config=None):
        ...
        self.filter_cols = FilterColumnsPostprocess(config={"filter_cols.drop": ["Postprocesses", "Frontend", "Framework", "Platform", "Comment", "Session", "ROM read-only", "ROM code", "ROM misc", "RAM data", "RAM zero-init data", "Run Stage Time [s]", "Compile Stage Time [s]", "Workspace Size [B]", "Build Stage Time [s]", "Load Stage Time [s]"]})
