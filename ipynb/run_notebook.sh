...
then
    ...
    then
        ...
        then
            ...
            python3 -m mlonmcu.cli.main init -t $YAML $WORKSPACE --non-interactive --clone-models
        else
            echo "(No template found. Falling back to default...)"
            python3 -m mlonmcu.cli.main init -t default $WORKSPACE --non-interactive --clone-models
        fi
