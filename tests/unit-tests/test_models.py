...
def _check_summary_output(out, expected_paths=[], expected_models=[], expected_groups={}, expected_skipped=0, expected_metadata_count=0, expected_duplicates=0, expected_group_duplicates=0, detailed=False):
    expected_paths_str = ["    " + str(path) for path in expected_paths]
    expected_models_str = ["    " + name for name in expected_models]
    expected_groups_str = ["    " + name + " [{} models]".format(len(models)) for name, models in expected_groups.items()]
    skipped_count = 0
    duplicates_count = 0
    metadata_count = 0
    for line in out.split("\n"):
        ...
        if detailed:
            if line.startswith("        "):
                ...
                continue
        assert line in ["Models Summary", "Paths:", "Models:", "Groups:", ""] + expected_paths_str + expected_models_str + expected_groups_str
...
def test_models_print_summary(detailed, capsys, fake_context, fake_environment_directory, fake_config_home):
    ...
    with mock.patch.dict(fake_context.environment.paths, {"models": [PathConfig(env_models_dir), PathConfig(user_models_dir)]}) as mocked:
        print_summary(fake_context, detailed=detailed)
        out, err = capsys.readouterr()
        _check_summary_output(
                out,
