...
def get_parser(subparsers):
    ...
    parser.add_argument('DIR', nargs='?', type=str, default=get_environments_dir(),
                        help="Environment directory (default: " + os.path.join(get_environments_dir(), "{NAME}") + ")")
    parser.add_argument('--non-interactive', dest="non_interactive", action='store_true', help="Do not ask questions interactively")
    parser.add_argument('--venv', default=None, action='store_true', help="Create virtual python environment")
    parser.add_argument("--clone-models", default=None, dest="clone_models", action='store_true', help="Clone models directory into environment")
    parser.add_argument("--register", default=None, action='store_true', help="Add environment to the list of environment for the current user")
    return parser
...
def handle(args):
    ...
    initialize_environment(args.DIR, name, create_venv=args.venv, interactive=not args.non_interactive, clone_models=args.clone_models, register=args.register, template=args.template)
