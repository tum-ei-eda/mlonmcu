...
def add_flow_options(parser):
    ...
    flow_parser.add_argument("-t", "--target", type=str, metavar="TARGET", choices=SUPPORTED_TARGETS.keys(), default="etiss/pulpino", nargs=1, help="The target device/architecture (default: %(default)s choices: %(choices)s)")
    ...
    flow_parser.add_argument("-c", "--config",
                        metavar="KEY=VALUE",
                        nargs='+',
        action="append",
    ...
    flow_parser.add_argument(
        "-p", "--progress", action="store_true", help="Display progress bar (default: %(default)s)"
    )
    flow_parser.add_argument(
