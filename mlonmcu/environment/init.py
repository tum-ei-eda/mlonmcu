...
from .config import get_environments_dir, env_subdirs, get_config_dir, get_environments_file, DEFAULTS, init_config_dir
...
def clone_models_repo(dest, url="https://github.com/PhilippvK/mlonmcu-models.git"):  # FIXME: update URL
...
def initialize_environment(directory, name, interactive=True, create_venv=None, clone_models=None, register=None, template=None):
    ...
    if use_default_dir or register != False:
        if not os.path.exists(config_dir):
            print(f"The mlonmcu user config directory {config_dir} does not exist!", end=" - ")
    ...
    subdirs = env_subdirs
    if clone_models or (clone_models is None and ask_user("Clone mlonmcu-models repository into environment?", default=False, interactive=interactive)):
    ...
    if register or (register is None and ask_user("Should the new environment be added to your list of environments?", default=True, interactive=interactive)):
