...
class MyPostprocess(SessionPostprocess):
    ...
    def __init__(self, features=None, config=None):
        ...
        self.config2cols = Config2ColumnsPostprocess(config={"config2cols.limit": ["tvmllvm.desired_layout", "tvmaot.desired_layout", "tvmaotplus.desired_layout", "tvmrt.desired_layout", "xcorev.mem", "xcorev.mac", "xcorev.bi", "xcorev.alu", "xcorev.bitmanip", "xcorev.simd", "xcorev.hwlp"], "config2cols.drop": True})
        self.rename_cols = RenameColumnsPostprocess(config={"rename_cols.mapping": {"config_tvmllvm.desired_layout": "Layout", "config_tvmaot.desired_layout": "Layout", "config_tvmaotplus.desired_layout": "Layout", "config_tvmrt.drsired_layout": "Layout", "config_xcorev.mem": "XCoreVMem", "config_xcorev.mac": "XCoreVMac", "config_xcorev.bi": "XCoreVBi", "config_xcorev.alu": "XCoreVAlu", "config_xcorev.bitmanip": "XCoreVBitmanip", "config_xcorev.simd": "XCoreVSimd", "config_xcorev.hwlp": "XCoreVHwlp", "feature_autotuned": "Autotuned", "feature_debug": "Debug"}})
        self.features2cols = Features2ColumnsPostprocess(config={"features2cols.limit": ["autotuned", "debug"], "features2cols.drop": True})
        self.filter_cols = FilterColumnsPostprocess(config={"filter_cols.drop": ["Postprocesses", "Frontend", "Framework", "Platform", "Comment", "Session", "Run", "ROM read-only", "ROM code", "ROM misc", "RAM data", "RAM zero-init data", "Run Stage Time [s]", "Compile Stage Time [s]", "Workspace Size [B]", "Build Stage Time [s]", "Load Stage Time [s]"]})
